...
# Configure seaborn theme with minimalistic dark styling
_sns.set_theme(
    font_scale=1.1,
    rc={
        "figure.figsize": (10, 6),
...
# Color palettes for different chart styles - Minimalistic dark theme
# Convention across core plots:
#   colors[0] → benchmark
#   colors[1] → primary strategy
#   colors[2+] → additional series
_FLATUI_COLORS = [
    "#00e5ff",  # Benchmark: bright cyan
    "#ffff00",  # Strategy: bright yellow
    "#51cf66",  # Green
    "#ff6b6b",  # Red
    "#4dabf7",  # Blue
    "#845ef7",  # Purple
...
def plot_returns_bars(
    ...
    fig.suptitle(
        title, y=0.94, fontweight="bold", fontname=fontname, fontsize=14, color="#e0e0e0"
    )
    ...
    # Plot bars with series-distinct coloring
    if isinstance(df, _pd.DataFrame):
        # Distinct palette for first two series (Benchmark vs Strategy)
        series_pos_colors = {
            0: "#00e5ff",  # Benchmark positive
            1: "#ffff00",  # Strategy positive
        }
        series_neg_colors = {
            0: "#0ca3ad",  # Benchmark negative (muted cyan)
            1: "#b3b300",  # Strategy negative (muted yellow)
        }

        # Compute bar positions per category per series (grouped bars)
        n_series = len(df.columns)
        n_cats = len(df)
        total_width = 0.8
        bar_width = total_width / max(n_series, 1)
        indices = _np.arange(n_cats)

        for i, col in enumerate(df.columns):
            col_data = df[col].values
            pos_color = series_pos_colors.get(i, "#51cf66")
            neg_color = series_neg_colors.get(i, "#ff6b6b")
            colors_i = [pos_color if v >= 0 else neg_color for v in col_data]
            # Offset per series to form grouped bars
            offset = (i - (n_series - 1) / 2) * bar_width
            ax.bar(indices + offset, col_data, width=bar_width, color=colors_i, label=col, alpha=0.9)

        ax.set_xticks(indices)
        ax.set_xticklabels(df.index)
    else:
        # For Series, use conditional colors
        bar_colors = ['#51cf66' if val >= 0 else '#ff6b6b' for val in df]
        ax.bar(range(len(df)), df, color=bar_colors)
    ...
    ax.axhline(0, ls="--", lw=1, color="#404040", zorder=2)
    ...
    if ylabel:
        ax.set_ylabel(
            "Returns", fontname=fontname, fontweight="bold", fontsize=12, color="#e0e0e0"
        )
...
def plot_timeseries(
    ...
    if fill:
        if isinstance(returns, _pd.Series):
            ax.fill_between(returns.index, 0, returns, color=colors[1], alpha=0.3)
        elif isinstance(returns, _pd.DataFrame):
            for i, col in enumerate(returns.columns):
                ax.fill_between(
                    returns[col].index, 0, returns[col], color=colors[i + 1], alpha=0.3
                )
    ...
    ax.axhline(0, ls="-", lw=1, color="#404040", zorder=1)
    ax.axhline(0, ls="--", lw=1, color="#404040", zorder=2)
    ...
    if ylabel:
        ax.set_ylabel(
            ylabel, fontname=fontname, fontweight="bold", fontsize=12, color="#e0e0e0"
        )
    ax.yaxis.set_label_coords(-0.1, 0.5)
...
def plot_rolling_beta(
    ...
    if isinstance(returns, _pd.Series):
        hlcolor = "#404040" if grayscale else hlcolor
    ...
    if ylabel:
        ax.set_ylabel(
            "Beta", fontname=fontname, fontweight="bold", fontsize=12, color="#e0e0e0"
        )
        ax.yaxis.set_label_coords(-0.1, 0.5)
...
def plot_longest_drawdowns(
    ...
    colors = ["#ffff00", "#ffff00", "#ff6b6b"]  # Bright yellow for main line, red for highlights
    if grayscale:
        colors = ["#000000"] * 3
    ...
    highlight = "#404040" if grayscale else "#ff6b6b"
...
def plot_table(
    ...
    if title != "":
        ax.set_title(
            title, fontweight="bold", fontsize=14, color="#e0e0e0", loc=title_loc
        )
    ...
    for (row, col), cell in the_table.get_celld().items():
        ...
        # First column styling for vertical orientation
            ...
            cell.set_text_props(weight="bold", color="#e0e0e0")
        # Data row styling
        elif row > 1:
            cell.set_linewidth(1)
...
def plot_daily_heatmap(
    ...
    """
    Plot a month-by-day heatmap of daily returns with enhanced visualization.
    ...
    cbar : bool, default True
        Show color bar
    title : str, default "Daily Returns (%)"
        Plot title
    annot_size : int, default 6
    ...
    if grayscale:
        ...
        colors = ['#d73027', '#f46d43', '#fdae61', '#fee08b', '#ffffbf', '#d9ef8b', '#a6d96a', '#66bd63', '#1a9641']
        cmap = LinearSegmentedColormap.from_list("custom", colors, N=256)

    series = _pd.Series(returns).copy().fillna(0) * 100
    series.index = _pd.to_datetime(series.index)

    # Build pivot: rows = months (1-12), cols = days (1-31)
    df = _pd.DataFrame({"value": series})
    df["month"] = df.index.month
    df["day"] = df.index.day
    pivot = df.pivot_table(index="month", columns="day", values="value", aggfunc="mean")
    pivot = pivot.sort_index()
    
    # Ensure full 1-31 columns for alignment
    full_cols = _pd.Index(range(1, 32), name="day")
    pivot = pivot.reindex(columns=full_cols)
    
    # Create month-year labels based on actual data
    # Get unique month-year combinations from the data
    month_year_combinations = series.index.to_series().dt.to_period('M').unique()
    
    # Create labels for each month (1-12) based on the data
    month_labels = []
    for i in range(1, 13):
        # Find data for this month number
        month_data = series[series.index.month == i]
        if len(month_data) > 0:
            # Get the most recent year for this month
            year_for_month = month_data.index.year.max()
            month_labels.append(f"{_pd.Timestamp(2000, i, 1).strftime('%b')} {year_for_month}")
        else:
            # If no data for this month, use a placeholder
            month_labels.append(f"{_pd.Timestamp(2000, i, 1).strftime('%b')} -")
    ...
    # Create annotations for numbers inside squares
    annotations = pivot.round(1).astype(str)
    # Only show annotations for non-zero values to avoid clutter
    annotations = annotations.where(pivot != 0, "")
    ...
    # Make labels readable with simple colors
    _plt.yticks(rotation=0, fontsize=10, color="#b0b0b0")
    _plt.xticks(rotation=0, fontsize=9, color="#b0b0b0")

    if cbar:
        cbar_obj = hm.collections[0].colorbar
        cbar_obj.ax.tick_params(labelsize=10, colors="#e0e0e0")
        cbar_obj.set_label("Daily Return (%)", fontsize=12, fontweight='bold', color="#e0e0e0")

    # Add grid lines for better readability
    ax.set_xticks(_np.arange(0.5, 31.5, 1), minor=True)
    ax.set_yticks(_np.arange(0.5, 12.5, 1), minor=True)
...
def plot_monthly_bars(
    ...
    """
    Plot a bar chart of monthly returns with month-year labels.
    ...
    returns : pd.Series or pd.DataFrame
        Daily returns
    compounded : bool, default True
        Use compounded aggregation within month
    grayscale : bool, default False
    ...
    series = _pd.Series(returns).copy().fillna(0)
    series.index = _pd.to_datetime(series.index)
    
    # Aggregate to monthly returns
    if compounded:
        monthly = safe_resample(series, "ME", _stats.comp)
    else:
        monthly = safe_resample(series, "ME", "sum")
    
    # Get the last value of each month (end-of-month return)
    monthly = safe_resample(monthly, "ME", "last")
    monthly.index = _pd.to_datetime(monthly.index)
    
    # Create month-year labels
    monthly_labels = [f"{month.strftime('%b')} {month.year}" for month in monthly.index]
    ...
    # Plot bars with custom labels - green for positive, red for negative
    bar_colors = ['#51cf66' if val >= 0 else '#ff6b6b' for val in monthly]
    bars = ax.bar(range(len(monthly)), monthly, color=bar_colors)
    ax.set_xticks(range(len(monthly)))
    ax.set_xticklabels(monthly_labels, rotation=45, ha='right', color="#b0b0b0")
    ...
    if ylabel:
        ax.set_ylabel(
            "Monthly Return", fontname=fontname, fontweight="bold", fontsize=12, color="#e0e0e0"
        )
        ax.yaxis.set_label_coords(-0.1, 0.5)
